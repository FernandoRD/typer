...
├── drivers.py         — BaseKeyboardDriver, PynputDriver, EvdevDriver, get_keyboard_driver()
...
├── vault.py           — BitwardenVault, VaultCallbacks, VaultItem, is_available() (sem dependência de tkinter)
...
### `autotyper/drivers.py`

Camada de abstração para controle de teclado e interceptação de teclas de controle (`ESC` e `ENTER`).

| Símbolo | Tipo | Descrição |
| --- | --- | --- |
| `BaseKeyboardDriver` | Classe base (ABC) | Define a interface: `type_char`, `tap_enter`, `press_special_key`, `start_esc_listener`, `wait_for_enter_or_esc`, `close` |
| `PynputDriver` | Classe | Implementação baseada em `pynput.keyboard` (Windows, macOS e Linux X11) |
| `EvdevDriver` | Classe | Implementação nativa Linux via `/dev/uinput` e `/dev/input/event*` (Wayland) |
| `is_wayland() → bool` | Função | Detecta se a sessão gráfica atual é Wayland (`XDG_SESSION_TYPE` ou `WAYLAND_DISPLAY`) |
| `can_use_evdev() → bool` | Função | Valida se a biblioteca `evdev` está disponível e `/dev/uinput` tem permissão de escrita |
| `get_keyboard_driver() → tuple` | Função factory | Retorna `(driver_instance, "evdev" \| "pynput")` com detecção e fallback automático |
...
| Linux Wayland | Suportado | Driver nativo via evdev (uinput); ver WAYLAND.md |
